...
Админ-панель — только по /admin. /start админа в группу не пересылается,
а обычные сообщения админа уходят в группу как от клиента (например, чтобы создать заявку).
"""
...
Всё это также доступно кнопками в панели: /admin
